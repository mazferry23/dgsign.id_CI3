...
<head>
    ...
    <base href="<?=base_url('public/dgsign2020-brown/')?>"/>
    <!-- Site fevicon icons -->
...
<body>
    <main>
        ...
        if(!empty($audio)){
            ?>
        <audio id="bg_music">
            <source src="<?=base_url('public/uploads/'.$audio)?>" type="audio/mpeg"/>
        </audio>
         <!-- <audio id="bg_music">
        <source src="https://winst.xyz/invitation/media/sounds/Teddy-Swims-Youre-Still-The-One-Shania-Twain-Cover.mp3" type="audio/mpeg">
    </audio>  -->
      <section class="sound-toggle">
        ...
        <section class="intro" style="background-image: url('<?=site_url('public/uploads/'.$cover)?>');">
            <div class="cage-section">
                ...
                if(!empty($undangan) && isset($undangan->ivts_Name)){
                    ?>
                    <div class="guestname">
                    Yth Bpk/Ibu/Sdra/i<br>
                        <?=$undangan->ivts_Name?><br>
                        <?=$undangan->ivts_Address?>
                    </div>    
                    <div class="qrcode">
        ...
        <div class="main-function" data-aos="flip-down" style="display:yes">
                ...
                <div class="content">
                    ...
                    if(!empty($tanggalacara)){
                        try{
                            ...
                            $acara__ = $dateCountdown->format('d').' '.$dateCountdown->format('F').' '.$dateCountdown->format('Y');
                ...
                <div class="content">
                    <div class="content_inner">
                        ...
                        if(!empty($acara1)){
                           ...
                           //$nf = new NumberFormatter('en_US', NumberFormatter::ORDINAL);
                           $nf = new NumberFormatter('id_ID', NumberFormatter::ORDINAL);
                            ...
                            $acara__1 = $dateA->format('d').' '.$dateA->format('F').' '.$dateA->format('Y');
                        }
                        ?>
                        <h1 class="title"><?=@$data_acara1['judul']?></h1>
                        
                        <p><?php
                        //$dateA!=null ? $dateA->format('l') : ''
                        $day = $dateA->format('l');
                        if($day='Saturday'){
                            echo 'Sabtu';
                        }
                    ...
                    <div class="content_inner">
                        ...
                        if(!empty($acara2)){
                            try{
                                ...
                                $df = str_replace($dateA->format('d'),'',$df);
                                //echo $dateA->format('d').'<sup>'.$df.'</sup> '.$dateA->format('F').','.$dateA->format('Y');
                                $acara__2 = $dateA->format('d').' '.$dateA->format('F').' '.$dateA->format('Y');
                            }catch(Exception $e){
...
<script>
/*function myFunction() {
    var target = document.querySelector("#bg_music");
    target.play();
}
*/
</script>
