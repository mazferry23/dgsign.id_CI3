...
class Histori_wa extends Backend_Controller{
    ...
    public function datatables($id){
		...
		$this->datatables->select('inbox.inbox_Name, inbox.inbox_NoHp, inbox.inbox_Messages, inbox.inbox_Image,inbox.inbox_Date, inbox.inbox_Status');
		...
		$this->datatables->where('inbox_Client_Id',$id);
		$this->datatables->add_column(
			'tmp',
			function($row){
				return '<a href="'.site_url('public'.DS.'wa_tmp'.DS.$row['inbox_Image']).'" target="_blank"><img src="'.site_url('public'.DS.'wa_tmp'.DS.$row['inbox_Image']).'" width="30"/></a>';
			}
		);
		$this->datatables->add_column(
			'action',
